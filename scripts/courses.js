...
function displayCourses(filter = 'all') {
    ...
    filtered.forEach(c => {
        ...
        div.className = c.completed ? 'course-card completed' : 'course-card';
        div.textContent = `${c.subject} ${c.number}`;
        container.appendChild(div);
    });
    const total = filtered.reduce((sum, c) => sum + c.credits, 0);
    document.querySelector("#total-credits").textContent = `The total credits for course listed above is ${total}`;
}

document.querySelector(".filter-buttons").addEventListener('click', (e) => {
    if(e.target.tagName === 'BUTTON') displayCourses(e.target.id);
});
displayCourses();